...
def set_it_on_fire(input_string="very naughty boy") -> str:
    ...
    """
    newlist = list(input_string.upper())
    actualstring = "🔥"
    actualstring = actualstring + "🔥".join(newlist)
...
def best_letter_for_pets() -> str:
    ...
    for x in (the_alphabet):
        comparison[x] = pet_filter(x)
    newcomp = sorted(comparison,key=lambda key:len(comparison[key]),reverse = True)
    most_popular_letter = newcomp[0]
...
def random_filler_text(number_of_words=200) -> str:
    ...
    rand_first = random.randint(0,4)
    rand_second = random.randint(0,4)
    for x in range(number_of_words):
        rand_first = random.randint(3,7)
        rand_second = random.randint(0,3)
        words.append(my_dict[rand_first][rand_second])
    return " ".join(words)
...
def fast_filler(number_of_words=200) -> str:
    ...
    if os.path.isfile("dict_cache.json"):
        ...
        mode = "r"
        newwriting = open("dict_cache.json", mode)
        contents = json.load(newwriting)
        newwriting.close()
    for x in range(number_of_words-1):
        rand_first = random.randint(3,7)
        rand_second = random.randint(0,3)
        words.append(contents[f"{rand_first}"][rand_second])
    words.append(".")
